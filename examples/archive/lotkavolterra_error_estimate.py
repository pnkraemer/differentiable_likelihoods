...
class InvProblemLklhd():
    ...
    def lklhdeval(self, theta):
        ...
        return lklhd * self.prioreval(theta)
    ...
    def gradeval(self, theta):
        ...
        # print(grad)
        return  1e-1 * (grad) * self.prioreval(theta)
...
end_time = 10
ivpnoise = 0.0000
thetatrue = np.array([1.5, 0.05, 0.05, 1.5])



# Set Method Parameters
q = 1
h = (end_time - initial_time)/1000
nsamps = 50
init_theta = np.array([1.5003, 0.0503, 0.0503, 1.5003])
...
bestguess_lang = lang_thetas[np.argmax(lang_guesses)]
bestguess_rw = rw_thetas[np.argmax(rw_guesses)]

disc_lang = np.linalg.norm((thetatrue - bestguess_lang)/thetatrue)/4
disc_rw = np.linalg.norm((thetatrue - bestguess_rw)/thetatrue)/4

print("Best guess lang:", bestguess_lang, np.amax(lang_guesses))
print("Best guess rw:", bestguess_rw, np.amax(rw_guesses))


print("Weighted error lang: %.1e" % disc_lang)
print("Weighted error rw: %.1e" % disc_rw)
