...
class InvProblemLklhd():
    ...
    def gradeval(self, theta):
        ...
        return 0.1 * grad * self.prioreval(theta)
    ...
    def forwardsolve(self, theta):
        ...
        mean = odesolver.get_trajectory(m, 0, 0)[evalidcs]
        unc = odesolver.get_trajectory(v, 0, 0)[evalidcs]
...
def create_data(solver, ivp, thetatrue, stepsize, ivpnoise):
    ...
    evalpt = np.array(tsteps[evalidcs])
    means = odesolver.get_trajectory(m, 0, 0)
    data = means[evalidcs] + noise
    return evalpt, data
...
evalpt, data = create_data(solver, ivp, thetatrue, h, ivpnoise)
...
# Draw a grid
delta = 0.1
xpts = np.arange(1.5, 4, delta)
ypts = np.arange(1.5, 4, delta)
...
plt.scatter(rw_thetas[:, 0], rw_thetas[:, 1], s=10, c=range(len(rw_thetas)), vmin=0, vmax=0.25*len(rw_thetas), cmap='Reds')
plt.scatter(lang_thetas[:, 0], lang_thetas[:, 1], s=10, c=range(len(rw_thetas)), vmin=0, vmax=0.25*len(lang_thetas), cmap='Blues')
