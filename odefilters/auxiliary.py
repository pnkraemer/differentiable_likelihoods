...
def array_shape_point_mean_covar(ptset, mean, covar):
    ...
    assert(isinstance(ptset, np.ndarray) is True), \
        "Please enter shape (N,d) ptset"
    assert(isinstance(mean, np.ndarray) is True), \
        "Please enter shape (d,) mean"
    assert(isinstance(covar, np.ndarray) is True), \
        "Please enter shape (d,d) covar"
    ...
    assert(ptset.shape[1] == mean.shape[0] == covar.shape[0]), \
