...
For ODE-based inverse problems, this module contains the
    NEGATIVE LOG-LIKELIHOOD,
that is, for likelihoods of the form l(z) = exp(-E(z)),
this module contains information about E():
    * Evaluations of E(z)
    * Evaluations of dE(z) (gradients)
    * Evaluations of d^2E(z) (Hessians)
    * Evaluations of (d^2E(z))^{-1}dE(z) (Newton steps)
Here, z is usually a parameter that one wants to infer.
...
class LogLikelihood():
    ...
    def loglklhdeval(self, par):
        """
        Evaluates NEGATIVE LOG LIKELIHOOD
        """
        self.mean, self.ipvar, self.jacob = self.forwardsolve(par)
        reshaped_loc = self.ipdata.data.flatten().reshape((1, len(self.ipdata.data.flatten())))
        return multivariate_potential()
        pass
...
def multivariate_potential(ptset, meanvec, covmat):
    """
