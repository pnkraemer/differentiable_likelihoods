...
def hamiltonian_nd(nsamps, eval_ptntl, init_state, stepsize,
                   ntraps, eval_gradptntl, eval_precond=None):
    """
    ...
    currstate, hessian = init_currstate(init_state, eval_ptntl, eval_gradptntl, eval_precond)
    ...
    for idx in range(nsamps):
        momtm = np.random.multivariate_normal(np.zeros(ndim), np.linalg.inv(hessian))
        curr_hamilt = evaluate_hamiltonian(momtm, currstate)
        momtm, proposal, hessian = compute_dynamics(momtm, currstate, ntraps,
                                           stepsize, eval_gradptntl, eval_ptntl, eval_precond)
...
def compute_dynamics(momentum, currstate, ntraps, stepsize, gradE, findE, eval_precond):
    """
    ...
    """
    proposal = MCMCState(currstate.state, currstate.loglkldeval, currstate.gradeval)
    for tau in range(ntraps):
        momentum, proposal, hessian = compute_next_trapstep(momentum, proposal,
                                                   stepsize, findE, gradE, eval_precond)
    # proposal.loglkldeval = findE(proposal.state)
    # print(proposal)
    return momentum, proposal, hessian


def compute_next_trapstep(momentum, proposal, stepsize, findE, gradE, eval_precond):
    """
    ...
    """
    # print("gradeval", proposal.gradeval)
    momentum = momentum - stepsize * proposal.gradeval / 2.
    proposal.state = proposal.state + stepsize * momentum
    proposal.loglkldeval = findE(proposal.state)
    hessian = eval_precond(proposal.state)
    proposal.gradeval = np.linalg.solve(hessian, gradE(proposal.state))
    momentum = momentum - stepsize * proposal.gradeval / 2.
    return momentum, proposal, hessian
...
def evaluate_hamiltonian(momentum, state):
    ...
    """
    return 0.5 * momentum.T @ momentum + state.loglkldeval
...
def hamiltonian_accepted(hamilt_new, hamilt):
    ...
    # print(hamilt_diff)
    # print(hamilt_diff)
    if hamilt_diff < 0 or -np.log(np.random.rand()) < hamilt_diff:
        # print("Accet")
        return True
    else:
        return False
...
def init_currstate(init_state, statlogpdf, grad, evalprecond=None):
    """
    ...
    loglkldeval = statlogpdf(init_state)
    if grad is not None:
        gradeval = grad(init_state)
        hesseval = evalprecond(init_state)
        currstate = MCMCState(init_state, loglkldeval, np.linalg.solve(hesseval, gradeval))
        # print("cstate", currstate)
    else:
        currstate = MCMCState(init_state, loglkldeval, 0)
    return currstate, hesseval
